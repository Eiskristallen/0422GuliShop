  ...
  <div class="trade-container">
    ...
    <div class="money clearFix">
      <ul>
        <li>
          <b
            ><i>{{ tradeInfor.totalNum }}</i
            >件商品，总商品金额</b
          >
          <span>{{ tradeInfor.totalAmount }}</span>
        </li>
        <li>
          <b>返现：</b>
          <span>0.00</span>
    ...
    <div class="trade">
      <div class="price">
        应付金额: <span>{{ tradeInfor.totalAmount }}</span>
      </div>
    ...
    <div class="sub clearFix">
      ...
      <a href="javascript:;" class="subBtn" @click.prevent="submitPayment"
        >提交订单</a
      >
    </div>
  </div>
...
export default {
  ...
  methods: {
    getTradeInfor() {
      this.$store.dispatch('getTradeInfor');
    },
    ...
    async submitPayment() {
      ...
      let tradeNo = this.tradeInfor.tradeNo;
      //发请求,创建订单
      //会返回一个数据(订单编号)
      //携带该订单编号跳转到支付页面
      //需要收集请求需要的参数和tradeNo
      const result = await this.$Api.reqSubmitOrder(tradeNo, tradeInfo);
      //  如果返回code是200,则拿到订单编号,携带该订单编号跳转
      if (result.code === 200) {
        this.$alert('提交订单成功,即将跳转到支付页面');
        this.$router.push({
          path: '/Pay',
          query: {
            orderNo: result.data,
  ...
  computed: {
    //总数据
    ...mapState({ tradeInfor: (state) => state.Trade.tradeInfor }),
    //从总数据里面拿一些复杂数据进来,避免出现A.B.C的情况出现假报错
    detailArrayList() {
      return this.tradeInfor.detailArrayList || [];
    },
    userAddressList() {
      return this.tradeInfor.userAddressList || [];
    },
    //计算默认信息
    defaultAddress() {
